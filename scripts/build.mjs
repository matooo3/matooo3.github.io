...
<main id="main">
  <section class="hero wrap" aria-labelledby="hero-title">
    <div class="hero-top"><span class="eyebrow"><span class="status-dot"></span> APPS, KI & AUTOMATISIERUNG</span><span class="hero-location">${icon('map-pin')} Tübingen, Deutschland</span></div>
    ...
    <div class="hero-footer"><div><span>SOFTWARE</span><span>ARTIFICIAL INTELLIGENCE</span><span>FITNESS</span></div></div>
  </section>
  <section class="selected wrap section-space" id="projekte" aria-labelledby="projects-title"><div class="section-heading"><div><p class="eyebrow">01 / AUSGEWÄHLTE ARBEITEN</p><h2 id="projects-title">Was ich entwickle<span>.</span></h2></div><p>Vom ersten kleinen Tool bis zur KI-App.<br>Ein Einblick in das, woran ich arbeite.</p></div><div class="featured-grid">${featured}</div><div class="featured-grid additional-featured" id="more-featured">${additionalFeatured}</div><div class="featured-controls"><button class="button primary" id="show-more-projects" type="button" aria-expanded="true" aria-controls="more-featured" hidden><span>Mehr anzeigen</span> <small>8</small> ${icon('arrow-down')}</button><a class="button outline gallery-button" href="/projects/">Galerie öffnen ${icon('arrow-up-right')}</a></div><div class="secondary-project-link"><a class="text-link all-projects-link" href="#alle-projekte">Alle Projekte entdecken ${icon('arrow-down')}</a></div></section>
  <section class="about-section section-space" id="ueber-mich" aria-labelledby="about-title"><div class="wrap about-grid"><div><p class="eyebrow">02 / DER MENSCH DAHINTER</p><h2 id="about-title">Über mich<span>.</span></h2><p class="about-signature">Matze <span>/ aka matooo</span></p></div><div class="about-content"><p class="about-lead">Viele meiner Projekte entstehen, weil mir im Alltag eine passende App oder eine bestimmte Funktion fehlt. Dann baue ich sie selbst.</p><p>Ich studiere Informatik in Tübingen und beschäftige mich besonders mit KI und Agentic Engineering. Neben eigenen Apps entstehen dabei auch Projekte aus dem Studium und Experimente mit Open-Source-Modellen.</p><p>Neben der Informatik mache ich seit Jahren Calisthenics, fahre viel Fahrrad und im Winter Ski. Im FSJ habe ich mich zum Rettungssanitäter ausbilden lassen und war auch danach im Krankentransport und in der Notfallrettung tätig.</p><div class="interest-grid"><div>${icon('code')}<h3>Eigene Apps</h3><p>Anwendungen entwickeln, die ich selbst nutzen möchte.</p></div><div>${icon('barbell')}<h3>Calisthenics</h3><p>Seit Jahren ein fester Teil meines Alltags.</p></div></div></div></div></section>
  ...
  <section class="contact-section section-space" id="kontakt" aria-labelledby="contact-title"><div class="wrap"><div class="contact-top"><p class="eyebrow">05 / LASS UNS REDEN</p><span>Von einer Idee zum nächsten Projekt.</span></div><p class="contact-intro">Für Fragen, Austausch oder gemeinsame Projekte.</p><h2 id="contact-title"><a href="mailto:matze2948@gmail.com">Schreib mir gerne.${icon('arrow-up-right')}</a></h2><div class="contact-bottom"><div class="email-group"><a href="mailto:matze2948@gmail.com" class="contact-email">matze2948@gmail.com</a><button class="icon-button" id="copy-email" aria-label="E-Mail-Adresse kopieren" type="button">${icon('copy')}</button><span id="copy-status" class="sr-only" role="status"></span></div><a href="https://github.com/matooo3" target="_blank" rel="noopener noreferrer" class="contact-github">${icon('github-logo')} GitHub ${icon('arrow-up-right')}</a></div></div></section>
</main>
<footer class="site-footer wrap"><a class="wordmark" href="#" aria-label="Zurück nach oben">matze<span>.</span></a><p>© <span id="year">2026</span> Matze</p><a href="archiv/">Archiv ${icon('arrow-up-right')}</a><a href="#">Nach oben ${icon('arrow-up-right')}</a></footer>
</body></html>`;
